...
def dataframe_linkage(df):
    """ Linkage matrix of a Dataframe."""
    from scipy.cluster import hierarchy as hc
    return hc.linkage(df.values.T, method='average', metric='correlation')


def dataframe_dendrogram(df, **kwargs):
    """Dendrogram of a clustered dataframe."""
    from scipy.cluster import hierarchy as hc
    zaverage = dataframe_linkage(df)
    names = list(df.columns)
    hc.dendrogram(zaverage, labels=names, **kwargs)
    return zaverage, names


def cluster_labels(z, names, t):
