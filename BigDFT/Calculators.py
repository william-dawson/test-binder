...
class Runner():
    ...
    def __init__(self, **kwargs):
        import copy
        mro = type(self).mro()
        imro = mro.index(Runner)
        if not issubclass(object, type(self).mro()[imro + 1]):
            super().__init__(**kwargs)  # forward aspecific arguments
    ...
    def get_global_option(self, key):
        """
        Get one key in global options. The key must exist.
        Useful to force implementation of compulsory options when subclassing.
    ...
    def _run_options(self, **kwargs):
        ...
        """ :py:class`dict`: Complete options of process_run.
        ...
        # calculate the difference between the global options and local options
        self.local_options = {}
        """ :py:class`dict`: Local options of process_run.
         This dictionary can be accessed during the definition of the
         process_run method.
         It contains all the keys which have been overridden
         for definition of the runner.
        """
        for key, value in self.run_options.items():
            val = self._global_options.get(key)
            if val != value:
                self.local_options[key] = value
