...
def bigdft_to_psi4(sysA, sysB=None, chargeA=0, multiplicityA=None, chargeB=0,
                   multiplicityB=None):
    """
    ...
    # Figure out the multiplicity if not specified
    if multiplicityA is None:
        nel = 0
        for frag in sysA.values():
            nel += sum([x.atomic_number for x in frag])
        nel += chargeA
        if nel % 2 != 0:
            multiplicityA = 2
        else:
        ...
        pass
    if sysB is not None and multiplicityB is None:
        nel = 0
        for frag in sysB.values():
            nel += sum([x.atomic_number for x in frag])
        nel += chargeB
        if nel % 2 != 0:
            multiplicityB = 2
    ...
    strA = StringIO()
    write_xyz(sysA, strA)
    xyzA = str(chargeA) + " " + str(multiplicityA) + "\n"
    xyzA += "\n".join(strA.getvalue().split("\n")[2:])

    if sysB is not None:
        strB = StringIO()
        write_xyz(sysB, strB)
        xyzB = str(chargeB) + " " + str(multiplicityB) + "\n"
...
class PSI4Calculator(Runner):
    ...
    def pre_processing(self):
        ...
        chargeA = self.run_options.get("chargeA", 0)
        chargeB = self.run_options.get("chargeB", 0)
        multiplicityA = self.run_options.get("multiplicityA", None)
        multiplicityB = self.run_options.get("multiplicityB", None)
        ...
        if "sapt" in method:
            ...
            mol = bigdft_to_psi4(sysA=self.sys, chargeA=chargeA,
                                 multiplicityA=multiplicityA)
